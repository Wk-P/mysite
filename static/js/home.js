...
window.onload = () => {
    ...
    // preloadAudio(nameList);

    let BGMAuido = document.querySelector('.BGMAudio');
    let audioIndex = Math.floor(Math.random() * nameList.length);
    BGMAuido.src = `../static/audio/${nameList[audioIndex]}`;
    BGMAuido.addEventListener('ended', function () {
        var auido = this;
        auido.src = `../static/audio/${nameList[audioIndex]}`;
        auido.play();
        audioIndex = (audioIndex + 1) % nameList.length;
    });
