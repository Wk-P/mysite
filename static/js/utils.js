...
function pageAdd() {
    ...
    if (localStorage.getItem('pageList') === null) {
        pageList = [];
        localStorage.setItem("pageList", JSON.stringify([]));
    } else {
        pageList = JSON.parse(localStorage.getItem("pageList"));
    }
    console.log(pageList);
    let tabId = Math.random().toString(36).substring(2);
    if (!pageList.includes(tabId)) {
        pageList.push(tabId);
...
function pageDel(tabId) {
    ...
    console.log(pageList);
    let index = pageList.indexOf(tabId);
    console.log(index);
    if (index !== -1) {
